...
public class HandsDataFileReader {
	...
	public PokerSession readHands() throws TBException {
		if (f == null || !f.exists()) {
			throw new TBException("File '"
					+ (f != null ? f.getAbsolutePath() : "Unknow file")
					+ "' is unreadable or doesn't exists !");
		}

		PokerSession associatedSession = new PokerSession(f.getName()
				.split("-")[0], f.getName(), "Hold'em poker");
		...
		try {
			...
			while (currentLine != null) {
				try {
					...
					TrackerBoikLog.getInstance().log(
							Level.WARNING,
							"Impossible to read a hand in file '" + f.getName()
									+ "': '" + e.getMessage() + "'");
				} finally {
					while (currentLine != null
							&& !currentLine.contains(AppUtil.POKERSTARS)) {
						gotToNextLine();
					}
				}
			}
			...
			throw new TBException("(line:" + lineNo + ")"
					+ "Error while reading data in file '"
					+ f.getAbsolutePath() + "': '" + e.getMessage() + "'");
		}
	}

	/**
	 * Read hand and load data in memory for the associated session The data was
	 * not associated with controller until getting all information
	 * 
	 * @param br
	...
	private void readHand(PokerSession associatedSession) throws TBException {
		try {
			...
			Calendar date = AppUtil.parseCalendar(currentLine.split("-")[1]
					.trim().split("\\[")[0].trim());
			Double bbValue = Double.parseDouble(currentLine
					.split(AppUtil.CURRENCY)[2].split("\\)")[0].trim());
			if (date == null || handID.isEmpty()) {
				throw new TBException("(line:" + lineNo + ")"
						+ "Error in format of hand entry for line: '"
						+ currentLine + "'");
			}
	...
	/**
	 * -------------------------------------------- Read actions part analysis
	 * grammar module -----------------------------------
	 */

	/**
	 * ACTION = POKER_PLAYER: ACTION_KIND POKER_PLAYER = player_id ACTION_KIND =
	 * action_kind Read poker actions from the buffer and the current line
	 * 
	 * @param br
	 * @param currentLine
	 * @param preflop
	 * @return
	 */
	private List<PokerAction> readActions(Hand h, HandMoment hm)
			throws TBException, IOException {
		consumeUselessLines();
		...
		while (currentLine != null
				&& !(currentLine.contains(UNCALLED_BET)
						|| currentLine.contains(SUMMARY_COLLECTED)
						|| currentLine.contains(SHOWDOWN)
						|| currentLine.contains(HOLE_CARDS)
						|| currentLine.contains(FLOP)
						|| currentLine.contains(TURN) || currentLine
							.contains(RIVER))) {
			boolean actionLine = true;
			while (currentLine.contains(TIMED_OUT_PLAYER)
					|| currentLine.contains(CHAT_FLAG)
					|| currentLine.contains(CONNECTED_PLAYER)
	...
	/**
	 * Gets Amount of action kind switch action kind Folds return null
	 * 
	 * @param ak
	 * @return
	 */
	private Double readAmount(ActionKind ak) throws TBException, IOException {
		...
		default:
			try {
				String amountStr = currentLine.split(":")[1]
						.split(AppUtil.CURRENCY)[1].split(" ")[0].trim();
				res = Double.parseDouble(amountStr);
			} catch (Exception e) {
				throw new TBException("(line:" + lineNo + ")"
						+ "Error while reading amount action in line: "
						+ currentLine);
			}
	...
	/**
	 * Just read the action kind in current line Throw error if no action kind
	 * are found or bad action kind id
	 * 
	 * @return
	 */
	private ActionKind readActionKindAndAllInFlag(Hand h,
			PokerPlayer currentPlayer) throws TBException, IOException {
		consumeUselessLines();
		ActionKind res = null;
		...
		if (actionKindStr.startsWith(ActionKind.CALL.getFileValue())) {
			...
			res = ActionKind.BET;
		} else if (actionKindStr.startsWith(ActionKind.POSTSBLIND
				.getFileValue())) {
			res = ActionKind.POSTSBLIND;
		} else if (actionKindStr.startsWith(ActionKind.POSTBIGBLIND
				.getFileValue())) {
			res = ActionKind.POSTBIGBLIND;
		} else {
			throw new TBException("(line:" + lineNo + ")"
					+ "Impossible to read actionKind in line: '" + currentLine
					+ "'");
		}
	...
	/**
	 * Just get the concerned player name, if synthax error in the expected line
	 * or if action player found is not on the list throw an error
	 * 
	 * @param players
	 * @return
	 */
	private PokerPlayer readActionConcernedPlayer(List<PokerPlayer> players)
			throws TBException, IOException {
		consumeUselessLines();
	...
	private String readAndConsumeTableName() throws IOException, TBException {
		try {
			...
			throw new TBException("(line:" + lineNo + ")"
					+ "Impossible to read table name, bad file format !");
		}
	}
	 ...
	 * @throws IOException
	 */
	private void addPlayerAndStackToHand(Hand h) throws TBException,
			IOException {
		consumeUselessLines();
		try {
			...
			while (currentLine != null && currentLine.startsWith(SEAT)) {
				String playerID = currentLine.split(":")[1].trim().split(
						" \\(" + AppUtil.CURRENCY)[0].trim();
				Double stack = Double.parseDouble(currentLine.split(" \\("
						+ AppUtil.CURRENCY)[1].split(" ")[0].trim());
			...
			throw new TBException("(line:" + lineNo + ")"
					+ "Error while parsing line for player and stack: "
					+ currentLine);
		}
	}

	/**
	 * Read Hand of hero player and add it to the hand throw exception if hand
	 * is not correctly formatted
	 * 
	 * @param players
	 */
	private void addAndConsumeHeroPlayerHand(Hand h) throws TBException,
			IOException {
		consumeUselessLines();
		try {
			String player = currentLine.split("to")[1].split("\\[")[0].trim();
			String[] cards = currentLine.split("to")[1].split("\\[")[1]
					.replace("]", "").trim().split(" ");

			PokerPlayer hero = h.getPlayers().get(
					h.getPlayers().indexOf(new PokerPlayer(player)));
			if (hero == null) {
				throw new TBException("(line:" + lineNo + ")"
						+ "Impossible to retrieve hero's name in line: "
						+ currentLine);
			}
			...
			if (firstCard == null || secondCard == null) {
				throw new TBException("(line:" + lineNo + ")"
						+ "Impossible to read hero's hand in String: "
						+ currentLine);
			}
			...
			throw new TBException("(line:" + lineNo + ")"
					+ "Impossible to read hero player on line: " + currentLine);
		}
	}
	 ...
	 * @throws IOException
	 */
	private HandMoment readAndConsumeHandMomentBoard(Hand h)
			throws TBException, IOException {
		consumeUselessLines();
		try {
			HandMoment res = null;
			if (currentLine.contains(FLOP)) {
				res = HandMoment.FLOP;
				String[] flopStr = currentLine.split("\\[")[1].replace("]", "")
						.split(" ");
				...
				res = currentLine.contains(TURN) ? HandMoment.TURN
						: HandMoment.RIVER;
				PokerCard card = PokerCard.readCard(currentLine.split("\\[")[2]
						.replace("]", "").trim());
				...
				throw new TBException("(line:" + lineNo + ")"
						+ "Impossible to read hand moment in line: "
						+ currentLine);
			}
			...
			throw new TBException("(line:" + lineNo + ")"
					+ "Invalid moment line: " + currentLine);
		}
	}
	...
	private void readHandSummary(Hand h) throws TBException, IOException {
		...
		try {
			...
			if (currentLine == null) {
				throw new TBException("(line:" + lineNo + ")"
						+ "No summary found for hand '" + h.getId() + "'");
			}

			// Move to summary starts
			gotToNextLine();
			// Read Total Pot and Rake
			Double totalPot = Double.parseDouble(currentLine
					.split(AppUtil.CURRENCY)[1].split(" ")[0].trim());
			Double rake = Double.parseDouble(currentLine.split("\\|")[1]
					.split(AppUtil.CURRENCY)[1].split(" ")[0].trim());
			...
			throw new TBException("(line:" + lineNo + ")"
					+ "Error while parsing hand summary at line: "
					+ currentLine);
		}
	}

	/**
	 * Update player results contained in line for hand given in parameter PRE:
	 * Next current non empty line is a player result one
	 * 
	 * @param h
	 */
	private void updatePlayerResultsForHand(Hand h) throws TBException,
		...
		if (pp == null) {
			throw new TBException(
					"(line:"
							+ lineNo
							+ ")"
							+ "Impossible to retrieve player name in player result summary line: "
							+ currentLine);
		}
		...
		// Determine status
		if (currentLine.contains(SUMMARY_WON)
				|| currentLine.contains(SUMMARY_COLLECTED)) {
			hr = HandResult.WIN;
		} else if (currentLine.contains(SUMMARY_LOOSE)
				|| currentLine.contains(SUMMARY_MUCKED)) {
			hr = HandResult.LOOSE;
			...
			throw new TBException("(line:" + lineNo + ")"
					+ "Impossible to read result hand on line: " + currentLine);
		}
	...
	private void readPlayerCardsAndAmountWon(Hand h, PokerPlayer pp,
		...
		if (currentLine.contains(SUMMARY_SHOWED)
				|| currentLine.contains(SUMMARY_MUCKED)) {
			String toFind = currentLine.contains(SUMMARY_SHOWED) ? SUMMARY_SHOWED
					: SUMMARY_MUCKED;
			String[] cards = currentLine.split(toFind)[1].split("\\]")[0]
					.trim().replace("[", "").split(" ");
			PokerHand ph = new PokerHand();
			ph.setHand(PokerCard.readCard(cards[0]),
					PokerCard.readCard(cards[1]));
			h.setHandForPlayer(pp, ph);
		}

		if (hr.equals(HandResult.WIN)) {
			String toFind = currentLine.contains(SUMMARY_COLLECTED) ? SUMMARY_COLLECTED
					: SUMMARY_WON;
			Double amountWon = Double.parseDouble(currentLine.split(toFind)[1]
					.split(AppUtil.CURRENCY)[1].split("\\)")[0].trim());
	...
	private void consumeUselessLines() throws IOException {
		while (currentLine != null
				&& (currentLine.replace(" ", "").isEmpty()
						|| currentLine.contains(CHAT_FLAG) || currentLine
							.startsWith(HANDS_DOWNLOAD_FILE))) {
			gotToNextLine();
		}
	}
	...
	private void consummeItems(String elem) throws TBException, IOException {
		if (br != null && currentLine != null && currentLine.contains(elem)) {
			...
			throw new TBException("(line:" + lineNo + ")" + " Expected elem '"
					+ elem + "' not found, found '" + currentLine + "'.");
		}
	}
